...
function Client(options) {
  ...
  // Update the s3 creds if required
  this.updateS3 = (opts) => {
    this.s3 = opts.s3Client ||  new AWS.S3(opts.s3Options);
  }
...
function syncDir(self, params, directionIsToS3) {
  ...
  if (params.callBack && typeof params.callBack.durationInSeconds === 'number' && typeof params.callBack.definition === 'function') {
    setInterval(() => {
      params.callBack.definition(self);
    }, params.callBack.durationInSeconds * 1000);
  }
  findAllS3Objects();
  startFindAllFiles();
